...
{
    instance->extra_font_spacing = spacing;
}

uint16_t mgl_GetExtraSpacing(miagl_ptr instance)
{
    return instance->extra_font_spacing;
}

void mgl_ClearExtraSpacing(miagl_ptr instance)
{
    instance->extra_font_spacing = 0;
}
...
int16_t mgl_CalcTextWidth(miagl_ptr instance, const char* text, const mgl_glyph_t* font)
{
    ...
    while (*text) {
        ...
        if (index) {
            total_width += instance->extra_font_spacing;
...
{
    int16_t real_width = mgl_CalcTextWidth(instance, text, font);
